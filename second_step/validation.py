...
model_path = "../saved_models/prediction/"
...
datas = strong_enhancers + weak_enhancers
# datas = weak_enhancers    # 测试单独1个类别的预测能力
# Y = torch.ones(len(weak_enhancers))   # 测试单独1个类别的预测能力
X = torch.tensor(datas, dtype=torch.long)
Y = torch.cat((torch.zeros(len(strong_enhancers)), torch.ones(len(weak_enhancers))))
