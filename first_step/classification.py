...
class Encoder():
    def __init__(self, batch_size=128, lr=0.0001, num_epochs=100, seq_len = 200, model_name='classification', hidden=512, k_mer_size=5, \
             num_gpus=1, dropout=0.2, k_fold=20, enhancer_dir='../train_datas/classification/enhancer.txt', non_enhancer_dir='../train_datas/classification/non_enhancer.txt'):
        ...
        self.train_datasets, self.valid_datasets = self.get_k_fold_datasets(enhancer_dir, non_enhancer_dir)
    ...
    def load_checkpoints(self, k, epoch):
        checkpoint = torch.load(self.checkpoints_path + "model" + k + "_for_Kfold/" + self.model_name + "_epoch" + epoch + ".pth")
    ...
    def train(self):
        ...
        for k in range(K, self.k_fold):
            ...
            if is_load_checkpoint:
                ...
                train_losses, valid_losses, train_accs, valid_accs = checkpoint["train_losses"], checkpoint[
                    "valid_losses"], checkpoint["train_accs"], checkpoint["valid_accs"]
            ...
            for i in range(checkpoint_epoch, self.epoch):
                ...
                print(
                    ...
                    "Validation set loss: {:.4f}, sn:{:.4f}, sp:{:.4f}, mcc:{:.4f}, acc:{:.4f}".format(
                        k + 1, i, (end - start), train_loss / train_total_batch_num, train_sn, train_sp, train_mcc,
                        train_acc, valid_loss / valid_total_batch_num, valid_sn, valid_sp, valid_mcc, valid_acc))
                ...
                if is_save_moedl:
                    best_labels, best_probas = utils.stacked_tensor_to_np(valid_labels), utils.stacked_tensor_to_np(valid_probas)
                if self.early_stopping.early_stop:  # 达到早停止条件时，early_stop会被置为True
                    break  # 跳出迭代，结束训练

            utils.draw(train_losses, str(k + 1) + "fold_train_loss", self.output_dir, title="The train loss of " + str(k + 1) + " round model")
            utils.draw(train_accs, str(k + 1) + "fold_train_accuracy", self.output_dir, title="Accuracy of the " + str(k + 1) + " round training set")
            utils.draw(valid_losses, str(k + 1) + "fold_valid_loss", self.output_dir, title="The valid loss of " + str(k + 1) + " round model")
            utils.draw(valid_accs, str(k + 1) + "fold_valid_accuracy", self.output_dir, title="Accuracy of the " + str(k + 1) + " round validation set")
...
def main(argv=None):
    ...
    parser.add_argument("--model_name", default="classification", help="model name for checkpoint and output")
