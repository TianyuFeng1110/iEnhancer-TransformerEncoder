...
class Encoder():
    def __init__(self, batch_size=32, lr=0.0001, num_epochs=100, seq_len = 200, model_name='classification', hidden=512, k_mer_size=3, \
             num_gpus=1, dropout=0.2, k_fold=10, data_dir='../train_datas/classification/training.txt'):
        ...
        self.train_datasets, self.valid_datasets = self.get_k_fold_datasets(data_dir)
    ...
    def get_k_fold_datasets(self, data_dir):
        ...
        with open(data_dir, 'r') as file:
            ...
            for line in file.readlines():
                ...
                if len(line[1]) != 200:
                    ...
                    list.append(kmer_encoding(line[1].upper(), self.k_mer_size, self.seq_len, kmer_dict))
        ...
        print("sample szie: ", len(datas))
        X = np.array(torch.tensor(datas, dtype=torch.long))
        ...
        shuffled_labels = np.array(Y)[shuffle_indices]
    ...
    def save_models(self, i, train_losses, train_accs, valid_losses, valid_accs, k, counter, best_score):
        ...
        }, (self.checkpoints_path + "model" + str(k) + "_for_Kfold/" + self.model_name + "_epoch" + str(
            i + 1) + ".pth"))
    ...
    def train(self):
        ...
        for k in range(K, self.k_fold):
            ...
            for i in range(checkpoint_epoch, self.epoch):
                ...
                valid_sn, valid_sp, valid_mcc, valid_acc = utils.calculate_metrics2(sum(valid_tps), sum(valid_tns),
                                                                                    sum(valid_fps), sum(valid_fns))
                train_sn, train_sp, train_mcc, train_acc = utils.calculate_metrics2(sum(train_tps), sum(train_tns),
                                                                                    sum(train_fps), sum(train_fns))
                print(
...
def main(argv=None):
    ...
    parser.add_argument("--model_name", default="classification_without_gan", help="model name for checkpoint and output")
